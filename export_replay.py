...
def decisions(start: int, end: int) -> list[dict]:
    ...
    Only the B runs have these. This is the series that makes the point: it is
    the model's forecast, timestamped, next to what actually happened.
    """
...
def export_run(name: str) -> dict | None:
    ...
    return {
        "name": name,
        "arm": "predictive" if name.startswith("B") else "baseline",
        "t": grid,
...
def main(runs: list[str], out_name: str):
    ...
    out.write_text(json.dumps({
        "step_seconds": STEP,
        "scenario": "ramp 20 -> 80 req/s over 6 min, hold 4 min, ramp down",
        "runs": data,
    }, indent=1))
    for r in data:
